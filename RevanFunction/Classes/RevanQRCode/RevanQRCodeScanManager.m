...
@interface RevanQRCodeScanManager () <AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
@property (nonatomic, strong) AVCaptureSession *session;
@property (nonatomic, strong) AVCaptureVideoDataOutput *videoDataOutput;
@property (nonatomic, strong) AVCaptureVideoPreviewLayer *videoPreviewLayer;
@end

@implementation RevanQRCodeScanManager


static RevanQRCodeScanManager *_instance;
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    if (self.delegate && [self.delegate respondsToSelector:@selector(revan_QRCodeScanManager:didOutputMetadataObjects:)]) {
        [self.delegate revan_QRCodeScanManager:self didOutputMetadataObjects:metadataObjects];
    }
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(revan_QRCodeScanManager:brightnessValue:)]) {
        [self.delegate revan_QRCodeScanManager:self brightnessValue:brightnessValue];
    }
}
...
- (void)revan_palySoundName:(NSString *)name {
    ...
    /** 资源在命名空间的路径 */
    NSString *nibNamePath = [NSString stringWithFormat:@"%@.bundle/%@", @"RevanQRCode", name];
    NSString *audioFile = [currentBundle pathForResource:nibNamePath ofType:nil];
