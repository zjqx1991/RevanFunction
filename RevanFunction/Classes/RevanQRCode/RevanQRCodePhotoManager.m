...
@interface RevanQRCodePhotoManager () <UINavigationControllerDelegate, UIImagePickerControllerDelegate>
@property (nonatomic, strong) UIViewController *currentVC;
@property (nonatomic, strong) NSString *detectorString;
@end

@implementation RevanQRCodePhotoManager


+ (instancetype)sharedManager {
    static RevanQRCodePhotoManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[RevanQRCodePhotoManager alloc] init];
    });
    return manager;
}
...
- (void)imagePickerControllerDidCancel:(UIImagePickerController *)picker {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(cancelQRCodePhotoManager:)]) {
        [self.delegate cancelQRCodePhotoManager:self];
    }
}

- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    // 对选取照片的处理，如果选取的图片尺寸过大，则压缩选取图片，否则不作处理
    UIImage *image = [UIImage revan_imageSizeWithScreenImage:info[UIImagePickerControllerOriginalImage]];
    ...
    if (features.count == 0) {
        ...
        [self.currentVC dismissViewControllerAnimated:YES completion:^{
            if (self.delegate && [self.delegate respondsToSelector:@selector(qrCodePhotoManager:didFinishPickingMediaWithResult:)]) {
                [self.delegate qrCodePhotoManager:self didFinishPickingMediaWithResult:self.detectorString];
            }
        }];
    }
}
